...
1. **At session start, and every time the user says "continue" (including after a usage-limit gap):**
   ...
   - **This applies mid-session too, not just at the session-start checkpoint** (user rule, 2026-08-02): the moment a new item appears in REMINDER.md's Pending section - whether spotted during the start-of-session check or added by the user while work is already underway - triage it into SPEC.md (or BUGS.md for a defect) before the turn ends. Flagging it to the user is not a substitute for recording it; do both. Substantial/architecture items still get asked about per the rule above, but the flagging and the writing-it-down are two separate steps - don't skip the second one waiting for a reply to the first.
