...
export default async function BillsPage({
  ...
  if (preview) {
    ...
    return (
      <BillsClient
        ...
        upcomingByItemId={new Map()}
        paidByItemId={new Map()}
        currency="₱"
        previewMode
      />
    );
  ...
  const [{ data: bills, error }, overridesRes, balancesRes, settlementsRes, { forecast, currency }] =
    await Promise.all([
      supabase
  ...
  // T188: same "upcoming, excluding past-due" scoping T150/debt/savings
  // already settled on - a months-old missed payment showing as "next up"
  // would read as a bug, not a feature.
  const upcomingByItemId = groupBy(
    forecast.filter((row) => row.sourceType === "recurring" && row.type === "bill" && !row.pastDue),
    (row) => row.sourceId,
  );
