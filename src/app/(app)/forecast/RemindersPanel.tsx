...
export function RemindersContent({
  ...
  // precedent. Only offered for the active list; reordering completed
  // reminders has no real use.
  //
  // T230: was a pointer-based drag (a grip handle) until the user asked for
  // the same up/down click reorder every other list already uses - one
  // interaction to learn instead of two, and it works on touch without the
  // touch-action/scroll-capture care a drag needs inside a scrollable panel.
  const { orderedItems: orderedActive, moveUp, moveDown } = useOrderedList(
      ...
      {orderedActive.length === 0 ? (
        ...
        <div className="min-h-0 flex-1 overflow-y-auto pr-1">
          <ul className="divide-y divide-notion-hairline">
            {orderedActive.map((reminder, index) => (
              <li key={reminder.id} className="flex items-start gap-1 py-2 first:pt-0">
                {!readOnly && (
                  <div className="mt-0.5 shrink-0">
                    <ReorderButtons
                      onMoveUp={() => moveUp(reminder.id)}
                      onMoveDown={() => moveDown(reminder.id)}
                      isFirst={index === 0}
                      isLast={index === orderedActive.length - 1}
                    />
                  </div>
                )}
                <div className="min-w-0 flex-1">
