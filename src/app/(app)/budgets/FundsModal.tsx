    ...
    <Modal title={`${budgetName} - ${title}`} onClose={onClose}>
      ...
      >
        ...
        {/* T218: only shown when there's a real choice to make - a single
            connected account is used automatically. T284: field renamed to
            `balanceId` to match writeLedgerEntry's own form field
            (budgets/actions.ts) - a connected account is now just a regular
            `balances` row. */}
        {accounts.length > 1 && (
          <div>
