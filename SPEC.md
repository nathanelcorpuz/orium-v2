...
- [ ] **T170.** Leftover sample data is mixed into the production account, and it is settling alongside real money. *Found 2026-07-31 while measuring T151's impact. The account contains a budget named **"Groceries"** (allocation ₱6,000) linked to an income named **"Freelance — Aya"** - both verbatim rows from `seed_sample_data` (migration 0016), and the income's name still carries the em dash that T109/T113 removed from the seed. It sits alongside the user's own real "Groceries" budget linked to "Freelance - Sheena", so the Budgets page shows two identically-named budgets, and on 2026-06-05 and 2026-06-19 **both** replenished, producing ₱12,000 of the ₱26,000 T151 measured. In other words roughly half that overstatement is phantom, from sample rows that should not be in a production account at all.<br>Nothing was deleted: removing real-looking financial rows is destructive and the user has to confirm which of the two "Groceries" budgets is theirs. Needs a decision on scope too - a full audit for other seeded leftovers (the em-dash names are a reliable fingerprint, since T113 banned them everywhere else) rather than just this one pair. Do this **before** any historical balance correction under T151, since the correct amount depends on which rows are real.*
- [ ] **T169.** Make `seed_sample_data`'s dates relative to the seeding date instead of hardcoded January 2026. *Surfaced by T150: with past-due now kept, restoring sample data shows roughly seven months of past-due rows, because migration 0016 hardcodes `'2026-01-15'`-style start dates that are now in the past. Reached only via Settings > Restore sample data (typing "RESTORE"), never automatically at signup, so it is a wart rather than a blocker. Worth fixing on its own merits regardless: hardcoded dates make the sample family look more stale every month that passes, and by 2027 the entire dataset sits in the past. Fix is to anchor start/end/due dates to `current_date` in the seeding function while keeping the recurrence shapes (rent on the 1st, salary on the 15th and 30th, and so on) exactly as they are.*
