...
*Planned (T43) — future-dated spends become first-class:* an entry dated after today but inside the current cycle counts toward "spent this cycle"/progress/remaining everywhere those appear (Budgets page, Dashboard card, current-cycle forecast row), so the entries list and the totals always agree — today they disagree, logged as Bug #3. Each future-dated entry also renders as its own Forecast row on its `entry_date` (name from the budget + note, amount −entry amount, type budget), so the balance dips on the day the money actually leaves; the current-cycle row subtracts these from remaining so nothing double-counts. Edge cases (entries dated into a *future* cycle vs. that cycle's allocation row; whether these rows are editable) to be decided with the user at build time.
...
- [ ] **T43.** Future-dated spends as a first-class feature (user decision 2026-07-21, upgrading Bug #3 from display fix to feature): entries dated after today count in all cycle totals AND render as their own Forecast rows on their entry date — full scope in "Forecast integration" above. Plan the edge cases with the user before building.
