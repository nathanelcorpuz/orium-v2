-- T97 (SPEC.md Phase 16): sample-data seeding as an app feature, not just a
-- SQL-editor script. Two pieces:
--   1. `preferences.sample_data_seeded_at` - null until the account has been
--      auto-seeded once (first-ever login). Kept separate from whether the
--      account currently HAS sample data, so "Reset my data" (wipes
--      everything, leaves this column alone) doesn't cause the next login
--      to silently reseed a deliberately-emptied account.
--   2. `public.seed_sample_data(target_user uuid)` - the same T88 sample
--      dataset `supabase/seed.sql` inserts (see that file's own header for
--      the full story/rationale), rewritten to generate a fresh uuid per
--      row instead of using fixed ones, since this now needs to run safely
--      for every new signup, not one hand-run demo account. `SECURITY
--      DEFINER` so the existing supabase/seed.sql workflow (run from the
--      SQL editor as an admin, no `auth.uid()` session) still works
--      alongside the app's own calls (signup auto-seed, Settings' "Restore
--      sample data") - guarded so a regular authenticated user can only
--      ever seed their own account, not pass another user's id.
...
begin
  ...
  values
    -- Income
    (v_income_salary, target_user, 'Salary — Nathan', 'income', 3500000,
     '2026-01-15', '2030-12-31', 'net of taxes', v_balance_checking,
     1, 'month', null, array[15, 30], null, null, 'on_date'),
    (v_income_freelance, target_user, 'Freelance — Aya', 'income', 650000,
    ...
    (v_savings_college, target_user, 'Kids'' college fund', 'savings', -400000,
  ...
  insert into public.budget_entries (id, user_id, budget_id, entry_date, amount, note, direction) values
    (gen_random_uuid(), target_user, v_budget_groceries, '2026-06-05', 600000, 'Replenished from Freelance — Aya', 'incoming'),
    (gen_random_uuid(), target_user, v_budget_groceries, '2026-06-08', 285000, 'SM Hypermarket run', 'outgoing'),
    (gen_random_uuid(), target_user, v_budget_groceries, '2026-06-19', 600000, 'Replenished from Freelance — Aya', 'incoming'),
  ...
  insert into public.reminders (id, user_id, text, completed, completed_at) values
    ...
    (gen_random_uuid(), target_user, 'Review Q2 budget with Aya', true, '2026-06-15 10:00:00+08');
